<script lang="ts">
	...
	let title = $state(ticket.title);
	let description = $state(ticket.description);
	let priority = $state(ticket.priority);
	let status = $state(ticket.status);
	let assignedTo = $state(ticket.assigned_to?.id ?? '');
...
<AppShell title="Edit ticket" subtitle="Update {ticket.id.slice(0, 8)} and keep your team in sync.">
	{#snippet action()}
		<a href="/tickets/{ticket.id}" class="inline-flex h-10 items-center gap-2 rounded-xl border border-surface-200 bg-white px-4 text-sm font-semibold text-surface-700 shadow-sm hover:bg-surface-50 transition-colors">
			Cancel
		</a>
	{/snippet}

	<div class="mx-auto max-w-3xl">
		<a href="/tickets/{ticket.id}" class="-mt-4 mb-5 inline-flex items-center gap-1.5 text-sm font-medium text-surface-500 hover:text-primary-600 transition-colors">
